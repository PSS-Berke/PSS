...
export const processSummaryDataForCharts = (
  ...
  if (!summary || !summary.rows)
    return {
      ...
      topChartData: [],
      kpiSummary: {
        activeUsers: { sum: 0, avg: 0 },
        newUsers: { sum: 0, avg: 0 },
        screenPageViews: { sum: 0, avg: 0 },
        publisherAdClicks: { sum: 0, avg: 0 },
        publisherAdImpressions: { sum: 0, avg: 0 },
      },
      topDaysActivity: [],
  ...
  summary.rows.forEach((row) => {
    ...
    if (dateIndex !== -1) {
      ...
      const screenPageViews =
        metricValues[metrics.findIndex((m) => m.name === 'screenPageViews')] || 0;
      const publisherAdClicks =
        metricValues[metrics.findIndex((m) => m.name === 'publisherAdClicks')] || 0;
      const publisherAdImpressions =
        metricValues[metrics.findIndex((m) => m.name === 'publisherAdImpressions')] || 0;
  ...
  const dateChartData = Array.from(dateChartMap.values()).sort((a, b) =>
    a.name.localeCompare(b.name),
  );
  const topChartData = Array.from(topChartMap.values()).sort((a, b) =>
    a.name.localeCompare(b.name),
  );
  ...
  const avgPublisherAdImpressions =
    daysWithData > 0 ? totalPublisherAdImpressions / daysWithData : 0;
  ...
  const kpiSummary: KpiSummary = {
    ...
    publisherAdClicks: {
      sum: totalPublisherAdClicks,
      avg: parseFloat(avgPublisherAdClicks.toFixed(2)),
    },
    publisherAdImpressions: {
      sum: totalPublisherAdImpressions,
      avg: parseFloat(avgPublisherAdImpressions.toFixed(2)),
    },
  };
  ...
  const overallSummary: OverallSummaryItem[] = [
    ...
    {
      metric: 'Average daily screen page views/screens',
      value: parseFloat(avgScreenPageViews.toFixed(2)),
    },
  ...
  const adMetricsTrend: AdMetricsTrend[] = dailyActivityData
    .map((day) => ({
      date: day.date,
  ...
  // Calculated KPIs
  const newUsersShare =
    totalActiveUsers > 0 ? `${((totalNewUsers / totalActiveUsers) * 100).toFixed(2)}%` : '0%';
  const viewsPerUser =
    totalActiveUsers > 0 ? (totalScreenPageViews / totalActiveUsers).toFixed(2) : '0';
  const ctr =
    totalPublisherAdImpressions > 0
      ? `${((totalPublisherAdClicks / totalPublisherAdImpressions) * 100).toFixed(2)}%`
      : '0%';
