...
export const TopChartDataTable: React.FC<TopChartDataTableProps> = ({ topChartData }) => {
  const metrics = [
            ...
            {metrics.map((metric) => (
              ...
              >
                ...
                <div className="sm:w-1/2 text-right sm:text-left lg:text-center">
                  {data[metric.key as keyof ChartData] !== undefined
                    ? String(data[metric.key as keyof ChartData])
                    : 'N/A'}
                </div>
              </div>
            ))}
